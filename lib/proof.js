...
let AIRDROP_REWARD;
let AIRDROP_DEPTH;
let AIRDROP_SUBDEPTH;
let AIRDROP_LEAVES;
let AIRDROP_SUBLEAVES;
...
function createConstants(env) {
  const tree = env === 'production' ? require('../etc/tree.json') : require('../etc_dev/tree.json');
  AIRDROP_ROOT_ = tree.root;
  AIRDROP_REWARD = tree.reward;
  AIRDROP_DEPTH = tree.depth;
  AIRDROP_SUBDEPTH = tree.subdepth;
  AIRDROP_LEAVES = tree.leaves;
  AIRDROP_SUBLEAVES = tree.subleaves;

  const faucet = env === 'production' ? require('../etc/faucet.json') : require('../etc_dev/faucet.json');
  FAUCET_ROOT_ = faucet.root;
  FAUCET_DEPTH = faucet.depth;
  FAUCET_LEAVES = faucet.leaves;

  AIRDROP_ROOT = Buffer.from(AIRDROP_ROOT_, 'hex');
  FAUCET_ROOT = Buffer.from(FAUCET_ROOT_, 'hex');
  TREE_LEAVES = AIRDROP_LEAVES + FAUCET_LEAVES;
